...
export default function Home() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Profile/Portfolio Tab Switcher for Home Page */}
      <div className="flex justify-center gap-4 mt-12">
        <button
          onClick={() => setActiveTab('profile')}
          className={`px-6 py-2 rounded-full font-bold transition-all ${activeTab === 'profile' ? 'bg-white text-black' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
        >
          Profile
        </button>
        <button
          onClick={() => setActiveTab('portfolio')}
          className={`px-6 py-2 rounded-full font-bold transition-all ${activeTab === 'portfolio' ? 'bg-white text-black' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
        >
          Portfolio
        </button>
      </div>
      ...
      {activeTab === "portfolio" ? (
        ...
        <div className="animate-fade-in py-32 px-12 max-w-4xl mx-auto space-y-16">
          <h1 className="text-6xl font-serif">Aastha's Profile</h1>
          <p className="text-2xl text-white/70 leading-relaxed font-serif italic">
            A first-year student passionated about AI & ML, exploring the human-centric side of technology.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-12">
            <div className="space-y-4">
              <h3 className="text-teal-400 font-bold uppercase tracking-widest text-sm">Focus Areas</h3>
              <div className="flex flex-wrap gap-2 text-white/60">
                <span>AI & ML</span> • <span>UX Design</span> • <span>Brand Experience</span> • <span>Product Design</span>
              </div>
            </div>
            <div className="space-y-4">
              <h3 className="text-teal-400 font-bold uppercase tracking-widest text-sm">Education</h3>
              <p className="text-white/60">B.Tech CSE (AI & ML) - 1st Year student</p>
            </div>
          </div>
        </div>
      )}
